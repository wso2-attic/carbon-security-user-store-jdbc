...
public class JDBCIdentityStoreConnector extends JDBCStoreConnector implements IdentityStoreConnector {
    ...
    public User.UserBuilder getUser(String username) throws IdentityStoreException, UserNotFoundException {
        ...
        try (UnitOfWork unitOfWork = UnitOfWork.beginTransaction(dataSource.getConnection())) {
            ...
            try (ResultSet resultSet = namedPreparedStatement.getPreparedStatement().executeQuery()) {
                ...
                if (IS_DEBUG_ENABLED) {
                    log.debug(String.format("User with user id: %s retrieved from %s", userId, identityStoreId));
                }
    ...
    public List<User.UserBuilder> listUsers(String filterPattern, int offset, int length)
        ...
        try (UnitOfWork unitOfWork = UnitOfWork.beginTransaction(dataSource.getConnection())) {
            ...
            if (IS_DEBUG_ENABLED) {
                log.debug(String.format("%s users retrieved from: %s", userList.size(), identityStoreId));
            }
    ...
    public Map<String, String> getUserAttributeValues(String userId) throws IdentityStoreException {
        ...
        try (UnitOfWork unitOfWork = UnitOfWork.beginTransaction(dataSource.getConnection())) {
            ...
            try (ResultSet resultSet = namedPreparedStatement.getPreparedStatement().executeQuery()) {
                ...
                if (IS_DEBUG_ENABLED) {
                    log.debug(String.format("%s attributes of user %s retrieved from %s", userClaims.size(), userId,
                            identityStoreId));
                }
    ...
    public Group.GroupBuilder getGroup(String groupName) throws IdentityStoreException, GroupNotFoundException {
        ...
        try (UnitOfWork unitOfWork = UnitOfWork.beginTransaction(dataSource.getConnection())) {
            ...
            try (ResultSet resultSet = namedPreparedStatement.getPreparedStatement().executeQuery()) {
                ...
                if (IS_DEBUG_ENABLED) {
                    log.debug(String.format("Group with name %s retrieved from %s", groupName, identityStoreId));
                }
    ...
    public Group.GroupBuilder getGroupById(String groupId) throws IdentityStoreException {
        ...
        try (UnitOfWork unitOfWork = UnitOfWork.beginTransaction(dataSource.getConnection())) {
            ...
            try (ResultSet resultSet = namedPreparedStatement.getPreparedStatement().executeQuery()) {
                ...
                if (IS_DEBUG_ENABLED) {
                    log.debug(String.format("Group with id %s retrieved from %s", groupId, identityStoreId));
                }
    ...
    public List<Group.GroupBuilder> listGroups(String filterPattern, int offset, int length)
        ...
        try (UnitOfWork unitOfWork = UnitOfWork.beginTransaction(dataSource.getConnection())) {
            ...
            if (IS_DEBUG_ENABLED) {
                log.debug(String.format("%s groups retrieved for filter pattern %s from %s", groups.size(),
    ...
    public List<Group.GroupBuilder> getGroupsOfUser(String userId) throws IdentityStoreException {
        ...
        try (UnitOfWork unitOfWork = UnitOfWork.beginTransaction(dataSource.getConnection())) {
            ...
            try (ResultSet resultSet = namedPreparedStatement.getPreparedStatement().executeQuery()) {
                ...
                if (IS_DEBUG_ENABLED) {
                    log.debug(String.format("%s groups retrieved for user id %s from %s", groupList.size(), userId,
                            identityStoreId));
                }
    ...
    public List<User.UserBuilder> getUsersOfGroup(String groupId) throws IdentityStoreException {
        ...
        try (UnitOfWork unitOfWork = UnitOfWork.beginTransaction(dataSource.getConnection())) {
            ...
            try (ResultSet resultSet = namedPreparedStatement.getPreparedStatement().executeQuery()) {
                ...
                if (IS_DEBUG_ENABLED) {
                    log.debug(String.format("%s users retrieved for group %s from %s", userList.size(), groupId,
                            identityStoreId));
                }
